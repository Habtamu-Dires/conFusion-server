...
router.post('/login', cors.corsWithOptions,(req,res, next)=>{
  ...
  passport.authenticate('local',(err, user, info)=>{
    ...
    if(!user) {
      res.statusCode = 401;
      res.setHeader('Content-Type', 'application/json');
      res.json({success: false, statu: 'Login Unsuccessful!', err: info });

    }
    req.logIn(user, (err)=> {  //the passport.authenticate will add will add req.logIn method
      if(err) {
        res.statusCode = 401;
        res.setHeader('Content-Type', 'application/json');
        res.json({success: false, statu: 'Login Unsuccessful!', err: "Could not log in user" });
      }
